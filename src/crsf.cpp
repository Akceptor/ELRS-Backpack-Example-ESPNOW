...
void crsfReceive()
{
  ...
  if (espnow_received)  // flag
  {
    ...
    {
      const crsf_sensor_attitude_t *att = crsf.getAttitudeSensor();
      float pitch = (att->pitch / 10000.0f) * kRadToDeg;
      float roll = (att->roll / 10000.0f) * kRadToDeg;
      float yaw = (att->yaw / 10000.0f) * kRadToDeg;
      cur.hdg = yaw;
